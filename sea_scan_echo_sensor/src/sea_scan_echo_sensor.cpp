...
void SeaScanEchoSensor::HandleSonarResponseMsg()
{
    ...
    while (stopSerialPortThread == false)
    {
        ...
        if (msg.length() > 0)
        {
//            std::cout << "Just received msg " << msg << std::endl;
...
//                std::cout << "IMAGE" << std::endl;
            ...
            if (msg.find(DATA_MSG) != std::string::npos) 
            {
//                std::cout << "DATA";
                sensorMsgs.GetAltimeterDataFromMsg(msg, unfilteredRange, filteredRange);
//                std::cout << " Unfiltered Range " << unfilteredRange ;
//                std::cout << " Filtered Range " << filteredRange  << std::endl;
                ReportAltimeterRange();
            }
            else
            if (msg.find(ACK_MSG) != std::string::npos) 
            {
//                std::cout << "ACK" << std::endl;
            }
            else
            if (msg.find(NACK_MSG) != std::string::npos) 
            {
                ROS_INFO("NACK");
            }
            else
            if (msg.find(INFO_MSG) != std::string::npos) 
            {
//                std::cout << "INFO";
                sensorMsgs.GetDeviceInfoFromMsg(msg, sonarMajorSWVersion, sonarMinorSWVersion, sonarBetaSWVersion, sonarSerialNum);
//                std::cout << " Major " << sonarMajorSWVersion ;
//                std::cout << " Minor " << sonarMinorSWVersion ;
//                std::cout << " Beta " << sonarBetaSWVersion ;
//                std::cout << " SerialNum " << sonarSerialNum << std::endl;
                ReportSonarConfiguration();
            }
            else
            if (msg.find(POLO_MSG) != std::string::npos) 
            {
...
//std::cout << "in ReadFromSerialPort" << std::endl;
    ...
    while (stopSerialPortThread != true)
    {
        ...
        if (bytesread != -1)
        {
	        ...
	        //ROS_INFO("just read %d bytes. string is:%s", bytesread, tempstr.c_str());
            //ROS_INFO("partialMsgBuffer >%s< size is:%d", partialMsgBuffer.c_str(), (int)partialMsgBuffer.size() );
